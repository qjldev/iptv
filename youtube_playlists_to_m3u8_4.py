...
def find_git_root(start_dir: Path) -> Path | None:
    try:
        result = run_git_command(["rev-parse", "--show-toplevel"], start_dir)
...
def git_upload(script_dir: Path) -> None:
    git_root = find_git_root(script_dir)
    if git_root is None:
        print("Git upload skipped: this directory is not inside a git repository.", file=sys.stderr)
        return
    ...
    try:
        ...
        commit = run_git_command(["commit", "-m", timestamp], git_root, check=False)
        ...
        run_git_command(["push"], git_root)
    except FileNotFoundError:
        print("Git upload failed: git command was not found.", file=sys.stderr)
    except subprocess.CalledProcessError as exc:
        print("Git upload failed.", file=sys.stderr)
        if exc.stdout:
